...
import Animated, { runOnJS, useAnimatedReaction, useAnimatedScrollHandler, useDerivedValue, useSharedValue, withSpring } from "react-native-reanimated";
...
function DraggableFlatListInner(props) {
  ...
  useDerivedValue(() => {
    if (triggerReset.value) {
      runOnJS(reset)(); // Halt the in-flight pan gesture so any subsequent onUpdate/onEnd
