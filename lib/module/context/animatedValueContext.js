...
function useSetupAnimatedValues() {
  ...
  const rawTranslate = useSharedValue({
    x: 0,
    y: 0
  });
  ...
  const value = useMemo(() => ({
    ...
    viewableIndexMin,
    viewableIndexMax
  }), [activeCellOffset, activeCellSize, activeIndexAnim, containerSize, disabled, horizontalAnim, hoverAnim, hoverOffset, isDraggingCell, isTouchActiveNative, panGestureState, placeholderOffset, resetTouchedCell, scrollOffset, scrollViewSize, spacerIndexAnim, touchPositionDiff, touchTranslate, rawTranslate, triggerReset, autoScrollDistance, viewableIndexMin, viewableIndexMax]);
  useEffect(() => {
